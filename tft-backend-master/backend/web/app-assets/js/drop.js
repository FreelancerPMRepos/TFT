...
function setFstDropdown() {
    ...
    function createDropdown(select) {
        ...
        var dropdown = createFstElement("div", "fstdropdown", select.parentNode, {
            "click": openSelect,
            "blur": openSelect
        });
        ...
        var d = selected != undefined ? selected.text : "";
        dropdownPlaceholder.textContent =d;
        // console.log('3',dropdownPlaceholder.textContent);
        if (searchDisable == null || searchDisable != "true") {
            var search = createFstElement("div", "fstsearch", dropdown, null);
            createFstElement("input", "fstsearchinput", search, {
                "keyup": getSearch,
                "paste": getSearch,
                "blur": openSelect
            });
    ...
    function openSelect(event, open) {
        ...
        if (event.relatedTarget != null && event.relatedTarget.tagName == "INPUT" || event.target.tagName == "INPUT" && event.type != "blur" || event.target.tagName == "INPUT" && (event.relatedTarget != null && event.relatedTarget.className == "fstdropdown open") || event.target.classList.contains("fstselected") && event.type == "blur" && document.activeElement.classList.contains("fstsearchinput") || event.type == "blur" && document.activeElement.className == "fstlist") return;
    ...
    function rebindDropdown(select) {
        ...
        var selected      = select.options[select.selectedIndex];
        var d             = selected != undefined ? selected.text : ddList.firstChild.textContent;
        if (ddList.firstChild != null)
            select.fstdropdown.dd.querySelector(".fstselected").textContent = d;
    }
    ...
    function getSearch(event) {
        var pasteText = event.type != "paste" ? "" : typeof event.clipboardData === "undefined" ? window.clipboardData.getData("Text") : event.clipboardData.getData("text/plain");
