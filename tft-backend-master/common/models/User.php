...
class User extends \yii\db\ActiveRecord implements \yii\web\IdentityInterface
{
    ...
    { 
        ...
        /** @var User $user */
        $user = static::find()->joinWith(['userAdditionalInfos'])->where([
            '=',
            'user.id',
            $id
        ])
    ...
    {
        return $this->hasOne(\common\models\UserAdditionalInfo::className(), ['user_id' => 'id']);
    }
...
//        $this->access_token = Yii::$app->security->generateRandomString();
    ...
    public function getJWT()
    {
        ...
        $secret = static::getSecretKey();
        $currentTime = time();
        $expire = $currentTime + (86400 * 365); // 1 day
        $request = Yii::$app->request;
        $hostInfo = '';
