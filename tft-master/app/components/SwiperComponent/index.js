...
export default class SwiperComponent extends Component {
  ...
  componentDidMount() {
    ...
    if (index === 0 && this.animation1) {
      this.animation1.play(0, 55);
    }
    if (index === 2 && this.animation2) {
      this.animation2.play(58, 58);
      const dW = Dimensions.get('window').width;
      Animated.spring(this.state.scrollX, {
        toValue: dW * 2,
        duration: 600,
        delay: 500,
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (this.props.children !== prevProps.children) {
      if (this.props.loadMinimal && Platform.OS === 'ios') {
        ...
        // Commented to fix animation issues ---==>
        // this.setState(
    ...
    if (prevState.index !== this.state.index) {
      ...
      if (prevState.index === 0 && this.state.index > 0) {
        this.animation2.play(0, 58);
  ...
  updateIndex = (offset, dir, cb) => {
    ...
    console.log('Loop Jump ===> ', loopJump);
  ...
  renderPagination = () => {
    ...
    const dW = Dimensions.get('window').width;
    const paginationOpacity = this.state.scrollX.interpolate({
      inputRange: [dW, dW * 2],
      outputRange: [1, 0],
      extrapolate: 'clamp',
    });
  ...
  render() {
    ...
    const anim1Opacity = this.state.scrollX.interpolate({
      inputRange: [0, dW, dW * 2],
      outputRange: [1, 0, 0],
      extrapolate: 'clamp',
    });
    const anim1Translate = this.state.scrollX.interpolate({
      inputRange: [0, dW],
      outputRange: [1, 0.5],
      extrapolate: 'clamp',
    });

    const anim2Opacity = this.state.scrollX.interpolate({
      inputRange: [0, dW, dW * 2],
      outputRange: [0, 1, 1],
    });

    const anim2Translate = this.state.scrollX.interpolate({
      inputRange: [0, dW],
      outputRange: [0.5, 1],
      extrapolate: 'clamp',
    });

    const buttonY = this.state.scrollX.interpolate({
      inputRange: [dW, dW * 2],
      outputRange: [200, 0],
      extrapolate: 'clamp',
    });

    const buttonO = this.state.scrollX.interpolate({
      inputRange: [dW, parseInt(dW * 2), parseInt(dW * 3)],
      outputRange: [0, 1, 1],
      extrapolate: 'clamp',
    });
